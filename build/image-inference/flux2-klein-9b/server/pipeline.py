...
class Flux2Klein9BPipeline:
    ...
    quantization on the transformer and the Qwen3 text encoder — the same proven
    path as flux2-klein-4b, just pointed at the 9B checkpoint. Single-GPU
    serialization via lazy asyncio.Lock. One instance per process.

    Relies on the Jetson-built bitsandbytes in dustynv/vllm
    (libbitsandbytes_cuda129.so with sm_87 cubin). Kept fully resident via
    pipe.to('cuda') exactly like the 4B build: on the Orin's unified memory CPU
    offload is pointless (cuda and cpu tensors share the same physical DRAM, so
    offloading just relabels bytes without lowering peak). Only the NF4
    quantization actually shrinks the footprint — whether the 9B fits resident
    in 16 GB is what this build is meant to find out."""
    ...
    def load(cls) -> "Flux2Klein9BPipeline":
        ...
        model_id = os.environ.get("FLUX_MODEL", "black-forest-labs/FLUX.2-klein-9B")
        nf4 = dict(
            load_in_4bit=True,
